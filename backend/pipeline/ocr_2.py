...
from invoices.repository import Invoice, write_invoice
...
def extract_markdown(pdf_bytes: bytes, invoice_id: str, invoice_name: str) -> tuple[str, int]:
    ...
    checkpoint_key = f"invoices:ocr:{invoice_id}"
    ...
    if checkpoint is not None:
        ...
        response = request_ocr(pdf_bytes, invoice_id, invoice_name, checkpoint_key)

    upload_file(f"{invoice_id}/ocr.json", response.model_dump_json().encode("utf-8"), "application/json")
    pages: list[str] = []
    for page in response.pages:
        ...
        for index, image in enumerate(page.images):
            ...
            upload_file(f"{invoice_id}/{filename}", base64.b64decode(encoded_image), "image/jpeg")
            markdown = markdown.replace(f"]({image.id})", f"](/api/documents/{invoice_id}/images/{filename})")
    ...
    markdown = "\n\n".join(pages)
    upload_file(f"{invoice_id}/document.md", markdown.encode("utf-8"), "text/markdown; charset=utf-8")
    return markdown, response.usage_info.pages_processed


def process(pdf_bytes: bytes, invoice_record: Invoice) -> None:
    with stage_attempt(invoice_record.id, invoice_record.name, "ocr", f"{invoice_record.id}/document.md"):
        _, pages = extract_markdown(pdf_bytes, str(invoice_record.id), invoice_record.name)
        invoice_record.pages = pages
        write_invoice(invoice_record)


def request_ocr(pdf_bytes: bytes, invoice_id: str, invoice_name: str, checkpoint_key: str) -> OCRResponse:
    encoded = base64.b64encode(pdf_bytes).decode("utf-8")
    with Mistral(api_key=os.environ["MISTRAL_API_KEY"], timeout_ms=OCR_TIMEOUT_MS) as client, track_usage(
        "mistral", OCR_MODEL, "ocr", invoice_id, invoice_name,
    ) as usage:
