...
class InvoiceErpTests(unittest.TestCase):
    def test_existing_document_snapshot_is_reused_without_reading_or_downloading(self) -> None:
        ...
        with patch("invoices.erp.get_client") as database, patch("invoices.erp.sync_erp_snapshot") as sync:
    ...
    def test_latest_saved_snapshot_is_bound_without_downloading(self) -> None:
        ...
        database = MagicMock()
        with (
            patch("invoices.erp.latest_snapshot_id", return_value=snapshot_id),
            patch("invoices.erp.get_client", return_value=database),
            patch("invoices.erp.sync_erp_snapshot") as sync,
        ):
            bind_snapshot(document)
    ...
    def test_first_snapshot_uses_upstream_sync_service(self) -> None:
        ...
        snapshot_id = uuid4()
        with (
            patch("invoices.erp.latest_snapshot_id", return_value=None),
            patch("invoices.erp.get_client"),
            patch("invoices.erp.get_redis"),
            patch("invoices.erp.sync_erp_snapshot", return_value=snapshot_id) as sync,
        ):
            bind_snapshot(document)
    ...
    def test_snapshot_created_by_another_worker_is_reused(self) -> None:
        ...
        snapshot_id = uuid4()
        with (
            patch("invoices.erp.latest_snapshot_id", side_effect=[None, snapshot_id]),
            patch("invoices.erp.get_client"),
            patch("invoices.erp.get_redis"),
            patch("invoices.erp.sync_erp_snapshot") as sync,
        ):
    ...
    def test_failed_snapshot_download_does_not_bind_document(self) -> None:
        ...
        with (
            patch("invoices.erp.latest_snapshot_id", return_value=None),
            patch("invoices.erp.get_client") as database,
            patch("invoices.erp.get_redis"),
            patch("invoices.erp.sync_erp_snapshot", side_effect=ConnectionError("ERP unavailable")),
        ):
    ...
    def test_only_unique_matches_from_the_bound_snapshot_are_linked(self) -> None:
        ...
        for entries, expected in [([], None), ([{"id": entry_id}], entry_id), ([{"id": entry_id}, {"id": str(uuid4())}], None)]:
            with self.subTest(entries=entries), patch("invoices.erp.get_client") as client:
    ...
    def test_missing_purchase_order_does_not_match_blank_erp_orders(self) -> None:
        document = queued_invoice()
        document.erp_snapshot_id = uuid4()
        with patch("invoices.erp.get_client") as client:
