...
class PipelineErpTests(unittest.TestCase):
    def test_existing_invoice_snapshot_is_reused_without_reading_or_downloading(self) -> None:
        invoice_record = queued_invoice()
        invoice_record.erp_snapshot_id = uuid4()
    ...
    def test_latest_saved_snapshot_is_bound_without_downloading(self) -> None:
        ...
        with (
            ...
            bind_snapshot(invoice_record)
        self.assertEqual(invoice_record.erp_snapshot_id, snapshot_id)
        ...
        database.table.return_value.update.return_value.eq.assert_called_once_with("id", str(invoice_record.id))
        sync.assert_not_called()
    ...
    def test_failed_snapshot_download_does_not_bind_invoice(self) -> None:
        invoice_record = queued_invoice()
        with (
            patch("pipeline.erp.latest_snapshot_id", return_value=None),
            ...
            with self.assertRaises(ConnectionError):
                bind_snapshot(invoice_record)
        database.assert_not_called()
        self.assertIsNone(invoice_record.erp_snapshot_id)
    ...
    def test_only_unique_matches_from_the_bound_snapshot_are_linked(self) -> None:
        ...
        for entries, expected in [([], None), ([{"id": entry_id}], entry_id), ([{"id": entry_id}, {"id": str(uuid4())}], None)]:
            with self.subTest(entries=entries), patch("pipeline.erp.get_client") as client:
                ...
                match_entry(invoice_record, "PO-123")
                query.eq.assert_called_once_with("snapshot_id", str(invoice_record.erp_snapshot_id))
