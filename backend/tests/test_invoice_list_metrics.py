...
from invoices.repository import Invoice, StageMetrics, list_invoices, write_invoice
...
class InvoiceListMetricsTests(unittest.TestCase):
    ...
    def test_completion_times_are_returned_with_each_invoice_page(self) -> None:
        client, redis = MagicMock(), MagicMock()
        rows = [{
        ...
        self.assertTrue(all(invoice_record.finished_at == datetime(2026, 9, 19, 10, 0, 20, tzinfo=timezone.utc) for invoice_record in result))
    ...
    def test_aggregated_metrics_are_not_written_into_invoices(self) -> None:
        invoice_record = Invoice(id=uuid4(), name="invoice.pdf", sha256="a" * 64,
