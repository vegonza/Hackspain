...
from invoices.batch import refresh_order_index
...
class BatchTests(unittest.TestCase):
    def test_changed_extraction_removes_stale_order_and_deleted_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            invoice_record = extracted / "scan"
            invoice_record.mkdir(parents=True)
            ...
            with patch("invoices.batch.extract_text", return_value="") as read:
                first = refresh_order_index(source, root / "index.json", extracted)
                self.assertEqual(first.other_invoices("PO-2026-0010", "other.pdf"), ["scan.pdf"])
                ...
                self.assertEqual(changed.other_invoices("PO-2026-0010", "other.pdf"), [])
                self.assertEqual(changed.other_invoices("PO-2026-0020", "other.pdf"), ["scan.pdf"])
                self.assertEqual(read.call_count, 2)
                (source / "scan.pdf").unlink()
                deleted = refresh_order_index(source, root / "index.json", extracted)
                self.assertEqual(deleted.invoices, {})
