...
def list_incidents() -> list[InvoiceIncident]:
    ...
    for row in rows:
        ...
        incidents.append(InvoiceIncident(
            ...
            due_date=payment_due_date(decision, row["invoice_date"], row.get("supplier_nif"), terms),
            amount_eur=euro_amount(decision, row.get("total_eur")),
...
def reset_invoice(invoice_id: UUID, name: str) -> None:
    ...
    client.table("documents").update({
        **fields, "exchange_rate": None, "status": "queued", "pages": 0, "erp_snapshot_id": None, "erp_entry_id": None, "payment_decision": None,
