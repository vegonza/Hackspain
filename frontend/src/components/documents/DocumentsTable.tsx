...
type Props = Pick<ReturnType<typeof useDocuments>, 'documentsLoading' | 'sortColumn' | 'sortDirection' | 'onToggleSort' | 'onUpload' | 'search' | 'onSearch' | 'onSelect' | 'onDocumentLink' | 'onDelete' | 'deleting' | 'labels'> & {
  ...
  onRedo: ReturnType<typeof useDocuments>['onRedo']
  redoDisabled: boolean
}

export function DocumentsTable({ documentsLoading, sortColumn, sortDirection, onToggleSort, onUpload, rows, search, onSearch, onSelect, onDocumentLink, onDelete, deleting, labels, onRedo, redoDisabled }: Props) {
  return (
        ...
        <table className="documents-table" aria-busy={documentsLoading}>
          ...
          <TableBody>
            {rows.map(document => <TableRow key={document.id} className="document-table-row" data-openable={document.canOpen} onClick={() => { if (document.canOpen) void onSelect(document.id) }}>
              ...
              <TableCell onClick={event => event.stopPropagation()}><div className="document-table-action">{document.canRedo && <ConfirmButton icon={RotateCcw} variant="redo" label={labels.redo} confirmation={document.redoConfirmation} disabled={redoDisabled} onConfirm={() => void onRedo(document.id)} />}{document.canDelete && <ConfirmButton icon={Trash2} variant="delete" label={labels.delete} confirmation={document.deleteConfirmation} disabled={deleting} onConfirm={() => void onDelete(document.id)} />}</div></TableCell>
            </TableRow>)}
