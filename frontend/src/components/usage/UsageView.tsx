...
export function UsageView({ mount, loading, failed, stats, daily, operations, records, pagination, pageKey, labels, totalLabel, failedPending, pendingError, onRetry, retrying }: ReturnType<typeof useUsage>) {
  return <section className="usage-view" ref={mount} aria-label={labels.title} aria-busy={loading}>
    ...
    <UsageLogTable {...{ loading, records, pagination, pageKey, labels, totalLabel }} />
  </section>
}
