...
export function UsageView({ mount, loading, failed, stats, daily, operations, records, pagination, pageKey, labels, failedPending, pendingError, onRetry, retrying }: ReturnType<typeof useUsage>) {
  return <section className="usage-view" ref={mount} aria-label={labels.title} aria-busy={loading}>
    <TableToolbar pagination={pagination} loading={loading}><h2>{labels.history}</h2></TableToolbar>
    ...
    <UsageDailyChart {...{ daily, operations, loading, labels }} />
    <UsageLogTable {...{ loading, records, pageKey, labels }} />
  </section>
}
