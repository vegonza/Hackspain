...
type Props = Pick<ReturnType<typeof useInvoices>, 'stageNavigation' | 'metricsLoading' | 'sourceTab' | 'onSourceTab'> & {
  label: string
  pdfLabel: string
  erpLabel: string
}

export function InvoicePipeline({ stageNavigation, metricsLoading, sourceTab, onSourceTab, label, pdfLabel, erpLabel }: Props) {
  return (
    <nav className="sidebar-navigation invoice-navigation" aria-label={label}>
      ...
      {stageNavigation.map(stage => {
        ...
        return <Button key={stage.id} variant="sidebar" size="sidebar" className={`sidebar-link ${sourceTab === stage.id ? 'bg-selected hover:bg-selected' : ''}`} aria-current={sourceTab === stage.id ? 'page' : undefined} onClick={() => onSourceTab(stage.id)}>
          ...
          <span className="invoice-stage-metrics" aria-busy={metricsLoading}>
