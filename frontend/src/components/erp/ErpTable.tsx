import { Building2, CircleAlert, Calendar, Euro, IdCard, Hash, ListChecks, Package, RefreshCw } from 'lucide-react'
...
export function ErpTable({ onRefresh, refreshing, loading, failed, rows, pagination, pageKey, search, onSearch, sortColumn, sortDirection, onToggleSort, onSelect, onEntryLink, labels }: Props) {
  const columns = [
    { column: 'entry', label: labels.entry, icon: Hash, width: '154px' },
    ...
    { column: 'date', label: labels.date, icon: Calendar, width: '150px' },
  ...
  return (
    <>
      ...
      <div className="invoices-table-scroll" key={pageKey}>
        <table className="invoices-table erp-table" aria-busy={loading}>
          ...
          <TableBody>
            {loading ? <TableSkeleton columns={columns.length} /> : rows.map(row => <TableRow key={row.id} className="invoice-table-row" data-openable="true" onClick={() => onSelect(row.id)}>
